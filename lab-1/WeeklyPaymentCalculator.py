...
C:\INF1002\Lab1\WeeklyPaymentCalculator> python3 weeklyPaymentCalculator.py 10000 10 200
...
def WeeklyPaymentCalculator():
    ...
    if extraHours > 0:
        normalSalary = 40 * normalRate
        extraSalary = extraHours * overtimeRate
    else:
        normalSalary = hours * normalRate
        extraSalary = 0

    totalSalary = normalSalary + extraSalary
    print(f"Normal Salary:{normalSalary:.2f}, Extra Salary: {extraSalary:.2f}, Total Salary: {totalSalary:.2f}")

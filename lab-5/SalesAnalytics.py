...
Running example: 
    C:\INF1002\Lab5\Sales> python3 SalesAnalytics.py 10,20,30,40,50,60 2
    ...
    The scaled numbers are:  [20.0, 40.0, 60.0, 80.0, 100.0, 120.0] The sorted sales numbers are:  [10, 20, 30, 40, 50, 60] The good sales numbers are:  [40, 50, 60]
'''
import sys
...
def scale(list1, x):
    newlist = list(map(lambda i: int(i * x), list1))
    return newlist
    
def sort(list1):
    newlist = sorted(list1, key=lambda i: i % 10)
    return newlist

def goodSales(list1):
    avg = sum(list1) / len(list1)
    newlist = list(filter(lambda i: i > avg, list1))
    return newlist
...
def SalesAnalytics():
    ...
    sales = list(map(int, sys.argv[1].split(",")))
    scale_factor = float(sys.argv[2])

    print(f"The scaled numbers are:  {scale(sales, scale_factor)} The sorted sales numbers are:  {sort(sales)} The good sales numbers are:  {goodSales(sales)}")
